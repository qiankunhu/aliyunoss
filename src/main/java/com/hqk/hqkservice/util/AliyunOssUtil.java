...
public class AliyunOssUtil {
    ...
    public static void main(String[] args) {
        ...
        System.out.println( String.format("%s/", dateStr) );
    }
    /**
     *
     * 上传图片
    ...
    public String uploadImg2Oss(MultipartFile file) {
        ...
        try {
            //上传文件
            SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
            String dateStr = String.format("%s/", format.format(new Date()) );

            String fileName=dateStr+name;

            InputStream inputStream = file.getInputStream();
            this.uploadFile2OSS(inputStream, fileName);
            return fileName;//RestResultGenerator.createSuccessResult(name);
        } catch (Exception e) {
    ...
    public String getImgUrl(String fileUrl) {
        if (!StringUtils.isEmpty(fileUrl)) {
            ...
            String url =  this.getUrl(this.fileDir + fileUrl);
    ...
    public String checkImage(MultipartFile file){
        ...
        System.out.println("上传图片后的路径:"+fileUrl);
    ...
    public  void deletePicture(String key){
        // 创建OSSClient实例。
        OSSClient ossClient  =  new OSSClient(endpoint, accessKeyId, accessKeySecret);
        ossClient.deleteObject( fileDir , key);
        // 关闭OSSClient。
        ossClient.shutdown();
    }
